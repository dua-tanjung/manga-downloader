<?php
    ...
    $scount = 0;
    $baselink = dirname($LINK).'/';
    foreach ($sels as $sel) {
        if ($sel->getAttribute("class")=="m" && $scount==0) {
            $opts = $sel->getElementsByTagName("option");
            foreach($opts as $opt) {
                $pnum = DOMinnerHTML($opt);
                $plink = $baselink.$opt->getAttribute("value").".html";
                if (strtolower($pnum) != "comments") {
    ...
    if ($rst == false) {
        $qry = "select pageid from pages where chapterid={$CHAPTERID} and PageNum='{$apage}'";
        $curpage = current($dbh->query($qry)->fetch());
    } else {
        $curpage=$rst['PAGEID'];
    }
    ...
    foreach($domimgs as $domimg) {
        if ($domimg->getAttribute("id")=="image")
